...
const ProfileUser = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const metaUrl = ((user.unsafeMetadata as any)?.avatarUrl as string) || "";
      const next = metaUrl || user.imageUrl || "";
      setAvatarUrl(next);
    })();
  }, [isLoaded, user]);
  ...
  const handleUpdate = async () => {
    try {
      ...
      if (clerkId) await updateUser(email, clerkId, firstName);
  ...
  return (
    <View style={{ paddingHorizontal: Spacing.lg, alignItems: "center" }}>
      <Avatar
        ...
        size={160}
        startUrl={avatarUrl}
        onChange={async ({ remoteUrl, localUri, type }) => {
          if (!user) return;

          try {
            const ext = (type && type.split("/")[1]) || "jpg";
            const file: any = { uri: localUri, name: `avatar.${ext}`, type: type || "image/jpeg" };
            ...
            try {
              ...
              console.log("Clerk setProfileImage failed (blob too):", e2);
            }
          }
          ...
          try {
            ...
            if (clerkId) {
              await updateUser(email, clerkId, firstName, { avatarUrl: remoteUrl });
            }
            await user.reload();

            setAvatarUrl(remoteUrl || user.imageUrl || "");
          } catch (err) {
            console.log("Avatar post-update failed:", err);
            setAvatarUrl(remoteUrl);
          }
        }}
      />

      <Text style={{ fontFamily: "bold", fontSize: Spacing.md, marginBottom: 4 }}>
        {firstName || "User"}
      </Text>
...
const styles = StyleSheet.create({
  ...
  verifyButtonText: { color: "white", fontFamily: "bold" },
});
